...
async function loginAction(formData: FormData) {
  ...
  if (!res.ok) {
    redirect('/login/mesa-entrada?error=1')
  }
...
export default async function MesaEntradaLoginPage({ searchParams }: { searchParams: Promise<{ error?: string, registration?: string }> }) {
  ...
  return (
    ...
    >
      ...
      />
      {/* Gradient overlay with teal theme */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'linear-gradient(135deg, rgba(20, 184, 166, 0.7) 0%, rgba(20, 184, 166, 0.3) 50%, rgba(20, 184, 166, 0.7) 100%)',
          zIndex: 1,
        }}
      />
      <div className="w-full max-w-md flex flex-col bg-white rounded-xl shadow-xl border border-gray-200 px-0 pt-0 pb-0 bg-opacity-95 backdrop-blur-sm relative" style={{zIndex:2}}>
        {/* Header específico para Mesa de Entrada */}
        <div className="bg-teal-600 rounded-t-xl px-6 py-5 flex flex-col items-center">
          <div className="mb-2">
            <svg className="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-white mb-1">Mesa de Entrada</h1>
          <p className="text-sm text-teal-100 mb-0">Acceso para recepción y atención</p>
        </div>
          ...
          <form action={loginAction} className="flex flex-col items-center">
            <div className="w-full max-w-sm flex flex-col">
              ...
              <input
                ...
                type="email"
                required
                className="block w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white text-gray-900 shadow-sm"
                placeholder="mesa@carelink.com"
              />
            </div>
            ...
            <button
              type="submit"
              className="mt-6 max-w-sm w-full sm:w-auto bg-teal-600 text-white py-2.5 px-8 rounded-lg hover:bg-teal-700 transition shadow-md font-semibold text-base"
            >
              Iniciar sesión
            </button>
          </form>
