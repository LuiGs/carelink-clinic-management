    ...
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden relative">
      ...
      >
        ...
        <div className="flex flex-col gap-2 p-4 py-6 overflow-y-auto overflow-x-hidden flex-1">
          <p
            className={cn(
              "px-2 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2",
          ...
          {MENU_ITEMS.map((tab, index) => {
            ...
            return (
              ...
              >
                <Icon
                  size={22}
                  className={cn("shrink-0", isActive ? "text-cyan-600" : "text-gray-500")}
                />
                <span
                  className={cn(
                ...
                {isActive && !isCollapsed && (
                  <div className="absolute right-0 top-0 h-full w-1 bg-cyan-600 rounded-l-full" />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-4 border-t bg-gray-50 md:hidden">
          {isAuthenticated ? (
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-gray-900 truncate max-w-35">
                    {user?.name || "Usuario"}
                  </span>
            ...
            <div className="flex flex-col gap-2">
              <Link
                href="/auth/login"
                className="flex items-center gap-3 text-gray-600 hover:text-cyan-700 hover:bg-cyan-50 p-2 rounded-md"
              >
                <LogIn size={20} />
